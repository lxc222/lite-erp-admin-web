...
const menu_auth_list = ref([]);

async function onLoadMenuList() {
    axiosGetJson("/api/open/biz/md/pub/getAuthMenu", function (response) {
        // console.info(response);
        if (!isAjaxRspSucc(response)) {
            menu_auth_list.value = [];
            MessagePlugin.error({
            ...
            menu_auth_list.value = response.data.data;
        }
    });
}
...
async function onRoleAddShowDlg() {
    onLoadMenuList();
    role_submit_model.value.submitData = {};
    role_submit_model.value.visible = true;
    role_submit_model.value.addMode = true;
}

async function onRoleEditShowDlg(editId) {
    onLoadMenuList();
    role_submit_model.value.submitData = {};
    role_submit_model.value.addMode = false;
    axiosPostJson(
        "/api/biz/md/authRole/oneById",
        {
            itemId: editId
        },
        function (response) {
            if (isAjaxRspSucc(response)) {
                role_submit_model.value.submitData = response.data.data;
                role_submit_model.value.visible = true;
            } else {
                MessagePlugin.error({
...
const onTableLineMenuHandler = data => {
    if ("修改信息" === data.content) {
        onRoleEditShowDlg(data.value.id);
    }
};

const onSubmitRole = () => {
    let distUrl = "/api/biz/md/authRole/add";
    if (role_submit_model.value.submitData.id > 0) {
        distUrl = "/api/biz/md/authRole/change";
    }

    axiosPostJsonBody(distUrl, role_submit_model.value.submitData, function (response) {
        if (!isAjaxRspSucc(response)) {
            MessagePlugin.error({
                content: response.data.tipMsg,
                duration: 3000,
            ...
            role_submit_model.value.visible = false;
            MessagePlugin.info({
...
<template>
    <t-card :bordered="true" :style="{ margin: '5px' }" id="query_form">
        <div style="padding-top: 10px; padding-bottom: 10px">
            ...
            >
                <t-form-item label="角色名称" name="角色名称">
    ...
    <t-card header-bordered :style="{ margin: '5px' }" id="data_table">
        <template #header>
            <div>
                <t-button theme="primary" variant="outline" @click="onRoleAddShowDlg()">
    ...
    <t-drawer
        id="role_submit_model"
        v-model:visible="role_submit_model.visible"
        @confirm="onSubmitRole"
        size="40%"
        placement="right"
        :closeBtn="true"
        :cancelBtn="null"
    ...
    >
        ...
        <t-form
            :rules="role_submit_model.rules"
            :data="role_submit_model.submitData"
            requiredMarkPosition="right"
        >
            <t-form-item label="角色名称" name="角色名称" help="*必填">
                <t-input placeholder="角色名称" v-model="role_submit_model.submitData.title" />
            </t-form-item>
            <t-form-item label="权限设置" name="权限设置" help="*必填">
                <t-tree
                    ref="tree"
                    v-model="role_submit_model.submitData.authCodes"
                    :data="menu_auth_list"
                    :checkable="true"
                    :checkStrictly="false"
                    :hover="true"
            ...
            <form_enum_radio_group
                label="是否启用"
                help="*必填"
                v-model="role_submit_model.submitData.status"
                v-if="globalLoginData.commonData"
                :options="globalLoginData.commonData.enableModel"
            />
            <t-form-item label="备注信息" name="备注信息">
                <t-input placeholder="备注信息" v-model="role_submit_model.submitData.remark" />
            </t-form-item>
        </t-form>
    </t-drawer>
</template>
