        ...
        children: [
            {
                ...
                title: "长度",
                cell: (h, { row }) => {
                    return <span>{row.cargoVolumeLength} 米</span>;
                }
            },
            {
                colKey: "cargoVolumeWidth",
                title: "宽度",
                cell: (h, { row }) => {
                    return <span>{row.cargoVolumeWidth} 米</span>;
                }
            },
            {
                colKey: "cargoVolumeHigh",
                title: "高度",
                cell: (h, { row }) => {
                    return <span>{row.cargoVolumeHigh} 米</span>;
                }
            }
        ]
    },
...
async function onShowEditDlg(editId) {
    ...
    axiosPostJson(
        "/api/biz/tms/deliveryVehicleSpec/oneById",
        {
            itemId: editId
        },
        function (response) {
...
const onSubmitData = ({ validateResult, firstError, e }) => {
    ...
    if (validateResult === true) {
        let distUrl = "/api/biz/tms/deliveryVehicleSpec/add";
        if (data_submit.value.submitData.id > 0) {
            distUrl = "/api/biz/tms/deliveryVehicleSpec/change";
        }
...
function onQueryFormData(resetPage) {
    ...
    LoadingPlugin(true);
    axiosPostJsonBody(
        "/api/biz/tms/deliveryVehicleSpec/page",
        {
...
<template>
    ...
    >
        <t-form
            ...
            @submit="onSubmitData"
        >
            <t-form-item label="规格名称" name="规格名称" help="*必填">
                <t-input
                    placeholder="规格名称"
