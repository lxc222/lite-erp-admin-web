...
import t_form_item_self_driver from "@/components/tms/t_form_item_self_driver.vue";
import t_form_item_tms_vehicle from "@/components/tms/t_form_item_self_vehicle.vue";
...
const table_columns = [
    { colKey: "id", title: "#ID", width: 100 },
    {
        ...
        title: "车辆信息",
        cell: (h, { row }) => {
            return <span>{row.vehicleTitle}</span>;
        }
    },
    { colKey: "driverTitle", title: "驾车司机" },
    {
        colKey: "statusTitle",
        title: "是否启用",
        width: 115,
...
async function onShowAddDlg() {
    data_submit_data.value.submitData = { status: globalConstDef.enableModel.Enabled };
...
async function onShowEditDlg(editId) {
    ...
    axiosPostJson(
        "/api/biz/tms/selfVehicleDriver/oneById",
        {
            itemId: editId
        },
        function (response) {
...
const onSubmitDataData = ({ validateResult, firstError, e }) => {
    ...
    if (validateResult === true) {
        let distUrl = "/api/biz/tms/selfVehicleDriver/add";
        if (data_submit_data.value.submitData.id > 0) {
            distUrl = "/api/biz/tms/selfVehicleDriver/change";
        }
...
<template>
    <t-card :bordered="true" :style="{ margin: '5px' }" id="query_form">
        <div style="padding-top: 10px; padding-bottom: 10px">
            ...
            >
                <t_form_item_tms_vehicle
                    label="车辆信息"
                    v-model="form_query_data.vehicleId"
                    :showFilter="true"
                />
                <t_form_item_self_driver
                    label="驾车司机"
                    v-model="form_query_data.driverId"
                    :showFilter="true"
                />
                <form_enum_list
                    label="是否启用"
    ...
    >
        ...
        >
            <t_form_item_tms_vehicle
                label="车辆信息"
                help="*必填"
                v-model="data_submit_data.submitData.vehicleId"
                :disabled="!data_submit_data.addMode"
            />
            <t_form_item_self_driver
                label="驾车司机"
                help="*必填"
                v-model="data_submit_data.submitData.driverId"
                :disabled="!data_submit_data.addMode"
            />
