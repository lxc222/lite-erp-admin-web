...
const table_columns = [
    ...
    //         return (
    //             <span>
    //                 1{row.title} =&gt; {row.convertRate} x {row.materialUnitTitle}
    //             </span>
    //         );
    //     }
    // },
...
const onSubmitUnitData = ({ validateResult, firstError, e }) => {
    ...
    if (validateResult === true) {
        let distUrl = "/api/biz/wms/stockUnit/add";
        if (unit_submit_data.value.submitData.id > 0) {
            distUrl = "/api/biz/wms/stockUnit/change";
        }
    ...
    >
        ...
        >
            ...
            <form_material_unit
                label="基本单位"
                help="*必填"
                v-model="unit_submit_data.submitData.materialUnitId"
