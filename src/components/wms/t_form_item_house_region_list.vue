...
import { loadWmsHouseRegionList } from "@/assets/asset_biz_wms_20251024.js";
...
watch(
    () => attrs.centerId,
    (newVal, oldVal) => {
        console.log("t_form_item_house_region_list watch:", newVal);
        model.value = null;
        onLoadData();
    }
);
...
function onLoadData() {
    ...
    let applyDepositFlag = !attrs.applyDepositFlag ? null : attrs.applyDepositFlag;

    if (showFilterAttr) {
        statusAttr = innerStatusFilter.value;
    }

    loadWmsHouseRegionList(
        {
            storeCenterId: centerId,
            status: statusAttr,
            applyDepositFlag: applyDepositFlag
        },
        response => {
