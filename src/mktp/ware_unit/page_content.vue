...
async function onShowUnitAddDlg() {
    unit_submit_data.value.submitData = { status: globalConstDef.enableModel.Enabled };
    unit_submit_data.value.visible = true;
    unit_submit_data.value.addMode = true;
}

async function onShowUnitEditDlg(editId, type) {
    unit_submit_data.value.submitData = {};
    unit_submit_data.value.addMode = false;
    axiosPostJson(
        "/api/biz/mktp/wareUnit/oneById",
        {
            itemId: editId,
            type: type
        },
...
const onTableLineMenuHandler = data => {
    if ("修改信息" === data.content) {
        onShowUnitEditDlg(data.value.id, data.value.type);
    }
};
...
const onSubmitUnitData = ({ validateResult, firstError, e }) => {
    ...
    if (validateResult === true) {
        let distUrl = "/api/biz/mktp/wareUnit/add";
        if (unit_submit_data.value.submitData.id > 0) {
            distUrl = "/api/biz/mktp/wareUnit/change";
        }
