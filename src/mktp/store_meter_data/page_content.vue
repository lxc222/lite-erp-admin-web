...
async function onMeterDataShowDeleteDlg(editId) {
...
async function onMeterDataDelete() {
    axiosPostJson(
        "/api/biz/mktp/storeMeterData/delById",
        {
...
const onTableLineMenuHandler = data => {
    if ("补充添加" === data.content) {
        onShowMeterDataCopyAdd(data.value);
    } else if ("作废记录" === data.content) {
        onMeterDataShowDeleteDlg(data.value);
    }
};

const onSubmitMeterDataAdd = ({ validateResult, firstError, e }) => {
    e.preventDefault();
    if (validateResult === true) {
        let distUrl = "/api/biz/mktp/storeMeterData/add";
...
function onQueryFormData(resetPage) {
    ...
    LoadingPlugin(true);
    axiosPostJsonBody(
        "/api/biz/mktp/storeMeterData/page",
        {
...
<template>
    ...
    <t-card header-bordered :style="{ margin: '5px' }" id="data_table">
        <template #header>
            <div>
                <t-button theme="primary" variant="outline" @click="onShowMeterDataAdd()">
        ...
        >
            <template #refOperation="{ row }">
                <t-dropdown
                    :options="[
                        { content: '补充添加', value: row.id, divider: true },
                        { content: '作废记录', value: row.id, divider: true }
                    ]"
    ...
    >
        ...
        >
            ...
            <t_form_item_mkp_cost_item
                ...
                :status="globalConstDef.enableModel.Enabled.toString()"
            />
            <t-form-item label="抄表日期" name="抄表日期" help="*必填">
                <t-date-picker
                    placeholder="抄表日期"
            ...
            <t-form-item label="抄表读值" name="抄表读值" help="*必填">
                <t-input
                    placeholder="抄表读值"
    ...
    >
        <t-form
            :data="meter_del_submit_data"
            requiredMarkPosition="right"
            @submit="onMeterDataDelete"
        >
