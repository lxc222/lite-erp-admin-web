...
const table_columns = [
    ...
    { colKey: "receiptNoPrefix", title: "单据前缀" },
...
const costItem_submit_data = ref({
    visible: false,
    addMode: false,
    submitData: {
        id: null,
...
async function onShowCostItemAddDlg() {
    costItem_submit_data.value.submitData = { status: globalConstDef.enableModel.Enabled };
    costItem_submit_data.value.visible = true;
    costItem_submit_data.value.addMode = true;
}

async function onShowCostItemEditDlg(editId) {
    costItem_submit_data.value.submitData = {};
    costItem_submit_data.value.addMode = false;
    axiosPostJson(
        "/api/biz/mktp/costItem/oneById",
        {
            itemId: editId
        },
        function (response) {
            if (isAjaxRspSucc(response)) {
                costItem_submit_data.value.submitData = response.data.data;
                costItem_submit_data.value.visible = true;
            }
        }
    );
...
const onTableLineMenuHandler = data => {
    if ("修改信息" === data.content) {
        onShowCostItemEditDlg(data.value.id);
    }
};

const onSubmitCostItemData = ({ validateResult, firstError, e }) => {
    e.preventDefault();
    if (validateResult === true) {
        let distUrl = "/api/biz/mktp/costItem/add";
        if (costItem_submit_data.value.submitData.id > 0) {
            distUrl = "/api/biz/mktp/costItem/change";
        }

        axiosPostJsonBody(distUrl, costItem_submit_data.value.submitData, function (response) {
            if (!isAjaxRspSucc(response)) {
                MessagePlugin.error({
                ...
                costItem_submit_data.value.visible = false;
                MessagePlugin.info({
...
<template>
    <t-card :bordered="true" :style="{ margin: '5px' }" id="query_form">
        <div style="padding-top: 10px; padding-bottom: 10px">
            ...
            >
                <t-form-item label="费用名称" name="费用名称">
    ...
    <t-card header-bordered :style="{ margin: '5px' }" id="data_table">
        <template #header>
            <div>
                <t-button theme="primary" variant="outline" @click="onShowCostItemAddDlg()">
    ...
    <t-dialog
        id="costItem_submit_data"
        v-model:visible="costItem_submit_data.visible"
        header="费用信息"
        :closeBtn="true"
        :cancelBtn="null"
        :confirmBtn="null"
    ...
    >
        <t-form
            :rules="costItem_submit_data.rules"
            :data="costItem_submit_data.submitData"
            requiredMarkPosition="right"
            @submit="onSubmitCostItemData"
        >
            <t-form-item label="费用名称" name="费用名称" help="*必填">
                <t-input placeholder="费用名称" v-model="costItem_submit_data.submitData.title" />
            </t-form-item>
            <form_enum_radio_group
                label="科目类别"
                v-model="costItem_submit_data.submitData.type"
                :disabled="!costItem_submit_data.addMode"
                help="*必填"
                v-if="thisModel.commonData"
            ...
            <form_enum_radio_group
                label="是否启用"
                v-model="costItem_submit_data.submitData.status"
                help="*必填"
                v-if="thisModel.commonData"
                :options="thisModel.commonData.enableModel"
            />
            ...
            >
                <t-input
                    placeholder="单据前缀"
                    v-model="costItem_submit_data.submitData.receiptNoPrefix"
                />
            </t-form-item>
            <t-form-item label="备注信息" name="备注信息">
                <t-input placeholder="备注信息" v-model="costItem_submit_data.submitData.remark" />
            </t-form-item>
            <t-form-item>
                <t-space size="small">
