...
const deposit_drawer_model = ref({
    visible: false,
    rack_data_list: [],
    rack_data_map: {},
    adjustOrderId: null,
...
function doSubmitRackAll() {
    let param = {
        orderId: deposit_drawer_model.value.adjustOrderId,
        remark: null,
        rackId2NumMap: deposit_drawer_model.value.rack_data_map
    };
    ...
    axiosPostJsonBody("/api/biz/wms/stockAdjust/submitById", param, function (response) {
        if (isAjaxRspSucc(response)) {
            ...
            deposit_drawer_model.value.visible = false;
        } else {
            MessagePlugin.error({
...
function onSubmitRackNew() {
    ...
    getOneWmsHouseRack(
        ...
        response => {
            ...
            deposit_drawer_model.value.rack_data_map[rack_new_input_model.value.rackId] =
                rack_new_input_model.value.num;

            deposit_drawer_model.value.rack_data_list.push({
            ...
            var totalStockNum = 0;
            deposit_drawer_model.value.rack_data_list.forEach(function (item, index) {
                totalStockNum += deposit_drawer_model.value.rack_data_map[item.rackId];
            });

            deposit_drawer_model.value.rackStockSum = totalStockNum;
        },
        () => {}
    );
}
...
function onDeleteRackExit(rackId) {
    var mapNum = deposit_drawer_model.value.rack_data_map[rackId];
    if (mapNum) {
        delete deposit_drawer_model.value.rack_data_map[rackId];
        deposit_drawer_model.value.rack_data_list =
            deposit_drawer_model.value.rack_data_list.filter(item => item.rackId !== rackId);
    }
}
...
watch(
    ...
    (newVal, oldVal) => {
        ...
        if (visible) {
            deposit_drawer_model.value.visible = true;
        } else {
            deposit_drawer_model.value.visible = false;
        }

        deposit_drawer_model.value.rack_data_list = [];
        deposit_drawer_model.value.rack_data_map = {};
        deposit_drawer_model.value.adjustOrderId = newVal.adjustOrderId;
        deposit_drawer_model.value.stockNum = newVal.stockNum;
        deposit_drawer_model.value.rackStockSum = 0;
        deposit_drawer_model.value.centerId = newVal.centerId;
        deposit_drawer_model.value.regionId = newVal.regionId;
    },
...
<template>
    <t-drawer
        id="deposit_drawer_model"
        v-model:visible="deposit_drawer_model.visible"
        size="40%"
        placement="right"
        :closeBtn="true"
        :cancelBtn="null"
        ...
        <t-card header-bordered :style="{ margin: '5px' }" id="data_table">
            <template #header>
                ...
                <div style="float: right; font-weight: bold">
                    <span>应入库数量：{{ deposit_drawer_model.stockNum }}</span>
                </div>
            </template>
            <template #footer>当前总数：{{ deposit_drawer_model.rackStockSum }}</template>
            <t-table
                row-key="index"
                :data="deposit_drawer_model.rack_data_list"
    ...
    >
        <t-form :data="rack_new_input_model" requiredMarkPosition="right">
            <t_form_item_house_rack_list
                ...
                :centerId="deposit_drawer_model.centerId"
                :regionId="deposit_drawer_model.regionId"
