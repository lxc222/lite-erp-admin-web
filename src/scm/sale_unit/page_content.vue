...
const table_columns = [
    ...
    },
    {
        ...
        cell: (h, { row }) => {
            return (
                <span>
                    1{row.title} =&gt; {row.convertRate} x {row.stockUnitTitle}
                </span>
            );
        }
    },
...
const onSubmitUnitData = ({ validateResult, firstError, e }) => {
    ...
    if (validateResult === true) {
        let distUrl = "/api/biz/scm/saleUnit/add";
        if (unit_submit_data.value.submitData.id > 0) {
            distUrl = "/api/biz/scm/saleUnit/change";
        }
